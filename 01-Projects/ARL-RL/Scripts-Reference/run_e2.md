...
#==============================================================================
#
#SBATCH --job-name=E2_Training
#SBATCH --output=e2_training_%j.out
#SBATCH --error=e2_training_%j.err
...
# Pass your account at submit time, e.g. via: sbatch --account=<your_account> scripts/run_e2.sh
...
# run_e2.sh — Stage E2 automation (Dueling DQN) for seeds 4,6,8
# Runs long trainings sequentially and appends results to e2_results.csv
# Usage:
#   sbatch scripts/run_e2.sh [--res 32|64] [--seeds "4 6 8"] [--episodes 3000]
# 
# SLURM Job Submission:
#   sbatch scripts/run_e2.sh
#   sbatch --time=06:00:00 --account=sbagchi scripts/run_e2.sh
#   sbatch --account=sbagchi --export=ALL,RL_SEED=4,RL_NUM_EPISODES=3000 scripts/run_e2.sh
...
# --- E2 training parameters (with optional env override) ---
# E2 = E1 + Dueling DQN + TUF=400
...
# Stage E2 flags:
#   - USE_DOUBLE_DQN=True (from E1, cosine LR scheduler)
#   - USE_DUELING_DQN=True (E2 addition)
#   - TARGET_UPDATE_FREQ=400 (E2: increased from 300)
echo "[CFG] E2 run: RES=$RES EPISODES=$EPISODES BATCH=${RL_BATCH_SIZE} REPLAY=100k LR=5e-5 EPS=(0.9→0.01 @100k) TUF=400 STEP_MUL=16 DUELING=1"

echo "[INFO] Seeds: ${SEEDS[*]}"

CSV="$RL_SAVE_PATH/e2_results.csv"
if [[ ! -f "$CSV" ]]; then
  echo "timestamp_utc,run_id,seed,lr,eps_decay,target_update,dueling_dqn,win_rate,avg_reward,artifacts_path" > "$CSV"
fi

for seed in "${SEEDS[@]}"; do
  export RL_SEED="$seed"
  export RL_RUN_ID="$(date -u +%Y%m%d_%H%M%S)_E2_seed${seed}"
  run_dir="$RL_SAVE_PATH/$RL_RUN_ID"
  mkdir -p "$run_dir"

  echo "[INFO] Starting E2 seed=$seed -> $RL_RUN_ID (RES=$RES)"
...
win_rate = avg_reward = lr = eps_decay = tuf = dueling = ''
try:
...
try:
    ...
    dueling = cfg.get('config', {}).get('USE_DUELING_DQN', '')
except Exception:
    pass
with open(csv_path, 'a') as f:
    f.write(f"{now},{run_id},{seed},{lr},{eps_decay},{tuf},{dueling},{win_rate},{avg_reward},{run_dir}\n")
...
echo "  - Standard output: e2_training_${SLURM_JOBID}.out"
echo "  - Standard error:  e2_training_${SLURM_JOBID}.err"
```